...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-[220px] shrink-0 flex-col border-r border-[var(--color-line)] bg-[color-mix(in_oklab,var(--color-paper),white_10%)] dark:bg-[var(--color-paper-2)]">
      <Link href="/app/today" className="flex items-center gap-2.5 px-4 h-12 border-b border-[var(--color-line)] hover:bg-[var(--color-warm-100)] dark:hover:bg-[var(--color-warm-800)] transition-colors">
      ...
      <nav className="flex-1 p-2.5">
        {NAV.map(({ href, label, icon: Icon, hint }) => {
          // exact match on /app, prefix match for nested routes
          const active = href === "/app" ? path === "/app" || path.startsWith("/app/programs") : path === href || path.startsWith(href + "/");
          return (
            <Link
              key={href}
              href={href}
