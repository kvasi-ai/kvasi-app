...
import { SEED_PROGRAMS, KINDS, AMOUNTS, LOCATIONS, type ProgramSeed } from "@/lib/programs-data";
...
export default function WorkspacePage() {
  ...
  const filtered: ProgramSeed[] = React.useMemo(() => {
    return SEED_PROGRAMS.filter((p) => {
  ...
  return (
    <div className="px-6 py-6 max-w-[1600px]">
      ...
      <div className="flex items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[13px] text-[var(--color-ink-2)] mt-1.5">
            ...
            <span className="tabular-nums">{SEED_PROGRAMS.length}</span> programs ·{" "}
      ...
      {/* legend */}
      <div className="flex items-center gap-5 text-[10.5px] text-[var(--color-ink-3)] mb-3 px-1">
...
function Sep() {
  return <span className="w-px h-4 bg-[var(--color-line)]" />;
}

function Lg({ sw, children }: { sw: string; children: React.ReactNode }) {
  return (
    <span className="flex items-center gap-1.5">
      <span className={cn("h-2 w-2 rounded-full", sw)} />
      {children}
    </span>
  );
}
